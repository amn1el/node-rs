...
import { init, t, setLocale, setFallback } from '../lib';
...
test('global:french', (tt) => {
  // FRENCH
  setLocale('fr-FR');
  setFallback('en-US');

  // Basic usage
  tt.is(t('common:hello'), 'Salut monde!');

  // With argument (replace #{name} to -> name: <val>)
  tt.is(t('common:user.name'), 'Bonjour, {{name}}!');
  tt.is(t('common:user.name', { name: 'Amniel' }), 'Bonjour, Amniel!');

  // With arguments
  tt.is(t('common:user.greeting'), 'Bonjour, {{name}}! Vous avez {{age}} ans et votre anniversaire est le {{birthday}}');
  tt.is(t('common:user.greeting', { name: 'Amniel', age: 21, birthday: '05/21' }), 'Bonjour, Amniel! Vous avez 21 ans et votre anniversaire est le 05/21');
